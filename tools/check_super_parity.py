#!/usr/bin/env python3
"""Audit the vendored SUPER core against the pinned upstream checkout.
...
described in ``super_planner_vendor/UPSTREAM.md``. A newly changed, missing, or
...
PORT_MANIFEST = VENDOR / "SUPER_PORT_SHA256.json"
...
def main() -> int:
    ...
    if not upstream.is_dir():
        print(
            f"upstream SUPER package does not exist: {upstream}", file=sys.stderr
        )
        return 2
    try:
    ...
    try:
        ...
        print(f"invalid SUPER port manifest: {error}", file=sys.stderr)
        return 2
    if manifest_commit != PINNED_COMMIT:
        print(
            f"SUPER port manifest pins {manifest_commit}; expected {PINNED_COMMIT}",
            file=sys.stderr,
        )
        return 1
    ...
    for relative in sorted(expected_manifest_files - manifest_paths):
        failures.append(f"missing SUPER port manifest hash: {relative}")
    for relative in sorted(manifest_paths - expected_manifest_files):
        failures.append(f"stale SUPER port manifest hash: {relative}")
    for relative in sorted(expected_manifest_files & manifest_paths):
        actual_digest = sha256(VENDOR / relative)
        if manifest_files[relative] != actual_digest:
            failures.append(
                f"reviewed SUPER port hash changed: {relative} "
    ...
    if failures:
        ...
        print(f"SUPER parity audit FAILED ({len(failures)} issue(s))", file=sys.stderr)
        return 1
    print(
        "SUPER parity audit OK at "
