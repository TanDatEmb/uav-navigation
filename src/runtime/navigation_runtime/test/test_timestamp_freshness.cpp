...
TEST(ExecutionStateFreshness, RequiresBothRosSourceAndSteadyReceiptToBeFresh) {
  ...
  const auto valid = navigation_interfaces::evaluateExecutionStateFreshness(
  ...
  const auto frozen_ros = navigation_interfaces::evaluateExecutionStateFreshness(
      now_ros, now_ros, now_steady, now_steady - 500'000'001LL, 0.5);
  EXPECT_EQ(frozen_ros.reason,
            navigation_interfaces::ExecutionStateFreshnessReason::kReceiveStale);

  const auto stale_source = navigation_interfaces::evaluateExecutionStateFreshness(
      now_ros, now_ros - 500'000'001LL, now_steady, now_steady, 0.5);
  EXPECT_EQ(stale_source.reason,
            navigation_interfaces::ExecutionStateFreshnessReason::kSourceStale);
}

TEST(ExecutionStateFreshness, FailsClosedForMissingFutureAndInvalidInputs) {
  using Reason = navigation_interfaces::ExecutionStateFreshnessReason;
  constexpr std::int64_t now_ros = 10'000'000'000LL;
  constexpr std::int64_t now_steady = 20'000'000'000LL;
  EXPECT_EQ(navigation_interfaces::evaluateExecutionStateFreshness(
