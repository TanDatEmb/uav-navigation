...
class MappingWorldModelTest : public ::testing::Test {
 protected:
  void SetUp() override {
    map = std::make_shared<navigation_mapping::RuntimeMappingMap>([] { return 1.0; });
    map->loadConfigAndInit(NAVIGATION_PLANNER_CONFIG_PATH);
    view = std::make_shared<navigation_mapping::MappingWorldModelView>(map);
  ...
  std::shared_ptr<navigation_mapping::RuntimeMappingMap> map;
  std::shared_ptr<navigation_mapping::MappingWorldModelView> view;
...
TEST_F(MappingWorldModelTest, SnapshotIdentityAndQueriesStayImmutableAfterMapUpdate) {
  ...
  const navigation_planning_backend::math::Pose pose{Eigen::Vector3d::Zero(), Eigen::Quaterniond::Identity()};
