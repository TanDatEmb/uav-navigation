...
namespace navigation_planning_backend {
    ...
    class LogOneReplan {
        ...
        // RET
        int ret_code{PLANNER_UNDEFINED};
    ...
    public:
        ...
        void reset() {
            ...
            total_t = 0.0;
            viz_t = 0.0;
            ret_code = PlannerResultCode::PLANNER_UNDEFINED;
            exp_traj.clear();
            exp_yaw_traj.clear();
            backup_traj.clear();
        ...
        void setGoal(const Vec3f &_goal_p, const double &_goal_yaw,
                     const navigation_math::RobotState &_robot) {
            robot_p = _robot.p;
        ...
        void replanBackupTrajectory(const BackupTrajOpt::Ptr &backup_traj_opt, Trajectory &traj) {
            if (exp_traj.empty() || backup_init_ps.size() == 0 || backup_init_t_vec.size() == 0
                || (backup_init_ps.size() !=
                    static_cast<std::size_t>(backup_init_t_vec.size()))
            ) {
