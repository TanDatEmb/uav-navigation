...
TEST(PlannerProductConfig, SatisfiesVisibilityInflationAndReplanBudgets) {
  ...
  EXPECT_DOUBLE_EQ(planner.control_envelope.maximum_velocity_mps, 5.0);
  EXPECT_DOUBLE_EQ(planner.control_envelope.maximum_acceleration_mps2, 5.0);
  EXPECT_DOUBLE_EQ(planner.control_envelope.maximum_jerk_mps3, 8.0);
  EXPECT_DOUBLE_EQ(planner.effective_cruise_speed_mps, 5.0);
  EXPECT_DOUBLE_EQ(planner.exp_traj_cfg.max_vel, 5.0);
  EXPECT_DOUBLE_EQ(planner.exp_traj_cfg.max_acc, 5.0);
  EXPECT_DOUBLE_EQ(planner.exp_traj_cfg.max_jerk, 8.0);
  ...
  EXPECT_DOUBLE_EQ(planner.yaw_rate_max_rad_s, 2.0);
  EXPECT_DOUBLE_EQ(planner.yaw_acceleration_max_rad_s2, 2.0);
...
TEST(PlannerProductConfig, RequestedCruiseIsGovernedByDefaultFiveMetresPerSecond) {
  for (const double speed_mps : {1.0, 3.0, 5.0, 7.0, 12.0}) {
    SCOPED_TRACE(speed_mps);
    navigation_planning::DynamicLimits mission;
    mission.intent.requested_cruise_speed_mps = speed_mps;
    navigation_planning_backend::Config planner(PLANNER_PRODUCT_CONFIG_PATH, mission);
    EXPECT_DOUBLE_EQ(planner.control_envelope.maximum_velocity_mps, 5.0);
    EXPECT_DOUBLE_EQ(planner.requested_cruise_speed_mps, speed_mps);
    const double effective_speed_mps = std::min(speed_mps, 5.0);
    EXPECT_DOUBLE_EQ(planner.effective_cruise_speed_mps, effective_speed_mps);
    EXPECT_DOUBLE_EQ(planner.exp_traj_cfg.max_vel, effective_speed_mps);
    EXPECT_DOUBLE_EQ(planner.exp_traj_cfg.max_acc, 5.0);
    EXPECT_DOUBLE_EQ(planner.exp_traj_cfg.max_jerk, 8.0);
    EXPECT_TRUE(planner.control_envelope.valid(planner.physical_model()));
    const double required_backup_horizon =
        navigation_planning_backend::jerkLimitedStopDistance(
            effective_speed_mps, planner.back_traj_cfg.max_acc,
            planner.back_traj_cfg.max_jerk) +
        2.0 * effective_speed_mps * planner.replan_forward_dt_s + planner.robot_r;
    EXPECT_DOUBLE_EQ(planner.visibility_horizon_m,
                     std::max(planner.visibility_horizon_floor_m,
                              required_backup_horizon));
    EXPECT_LE(planner.visibility_horizon_m, planner.visibility_horizon_cap_m);
  }
}
...
TEST(PlannerSpeedGovernor, FindsFeasibleSteadyCruiseBelowOldGridFloor) {
  navigation_math::StatePVAJ measured = navigation_math::StatePVAJ::Zero();
  navigation_planning::DynamicLimits dynamics;
  dynamics.vehicle.maximum_velocity_mps = 12.0;
  dynamics.vehicle.maximum_acceleration_mps2 = 12.0;
  dynamics.vehicle.maximum_jerk_mps3 = 30.0;
  dynamics.intent.requested_cruise_speed_mps = 5.0;

  // The former 16-point grid bottomed out at 0.3125 m/s. This scalar fixture
  // is intentionally not a corridor/world certificate: it only demonstrates
  // that the production polynomial family has a feasible point below that
  // grid floor under the caller's V/A/J limits.
  const auto governed = navigation_planning_backend::evidenceAwareSpeedLimit(
      measured, dynamics, {0.02, 0.02, 0.02, 0.02}, 0.05,
      [] { return false; });
  ASSERT_TRUE(governed.sufficient)
      << "failure=" << static_cast<int>(governed.failure);
  EXPECT_GT(governed.speed_mps, 0.0);
  EXPECT_LT(governed.speed_mps, 0.3125);

  // Independent numerical oracle for the specified positive steady-cruise
  // member. Evaluate the actual production polynomial and continuous extrema;
  // do not use the governor's duration/support helper as the test oracle.
  navigation_math::StatePVAJ oracle_state = navigation_math::StatePVAJ::Zero();
  oracle_state.col(1).x() = 0.1;
  const auto polynomial = navigation_planning_backend::minimumSnapStopPiece(
      oracle_state, 0.2);
  EXPECT_NEAR(polynomial.getMaxVelRate(), 0.1, 1.0e-10);
  EXPECT_NEAR(polynomial.getMaxAccRate(), 0.9375, 1.0e-9);
  EXPECT_NEAR(polynomial.getMaxJerRate(), 14.433756729740645, 1.0e-8);
  EXPECT_NEAR(
      navigation_planning_backend::minimumSnapStopSupportBound(
          polynomial, oracle_state.col(0)),
      0.01, 1.0e-10);
  EXPECT_TRUE(polynomial.getState(0.2).col(1).isZero(1.0e-9));
  EXPECT_TRUE(polynomial.getState(0.2).col(2).isZero(1.0e-8));
  EXPECT_TRUE(polynomial.getState(0.2).col(3).isZero(1.0e-7));
}

TEST(PlannerBackupBraking, CorrectsSteadyAccelerationDurationRoundDown) {
  navigation_math::StatePVAJ state = navigation_math::StatePVAJ::Zero();
  constexpr double speed = 0.08839407447163837;
  constexpr double acceleration_limit = 0.00012525139441244633;
  constexpr double jerk_limit = 1.0e6;
  state.col(1).x() = speed;
  const double rounded_lower_bound =
      navigation_planning_backend::minimumSnapSteadyCruiseDuration(
          speed, acceleration_limit, jerk_limit, 0.001);
  const auto uncorrected = navigation_planning_backend::minimumSnapStopPiece(
      state, rounded_lower_bound);
  ASSERT_GT(uncorrected.getMaxAccRate(), acceleration_limit);
  EXPECT_TRUE(navigation_planning::withinNumericalDynamicLimit(
      uncorrected.getMaxAccRate(), acceleration_limit));

  const auto seed = navigation_planning_backend::makeBackupBrakingSeed(
      0.0, state, speed, acceleration_limit, jerk_limit, 0.001, 0.0);
  ASSERT_TRUE(seed.feasible) << static_cast<int>(seed.failure);
  EXPECT_DOUBLE_EQ(seed.duration_s, std::nextafter(
      rounded_lower_bound, std::numeric_limits<double>::infinity()));
  EXPECT_LE(seed.maximum_acceleration_mps2, acceleration_limit);
  EXPECT_LE(seed.maximum_jerk_mps3, jerk_limit);
  EXPECT_TRUE(std::isfinite(seed.support_bound_m));
}

TEST(PlannerBackupBraking, CorrectsSteadyJerkDurationRoundDown) {
  navigation_math::StatePVAJ state = navigation_math::StatePVAJ::Zero();
  constexpr double speed = 0.028138831782626911;
  constexpr double jerk_limit = 4.2389422825644623;
  constexpr double acceleration_limit = 1.0e6;
  state.col(1).x() = speed;
  const double rounded_lower_bound =
      navigation_planning_backend::minimumSnapSteadyCruiseDuration(
          speed, acceleration_limit, jerk_limit, 0.001);
  const auto uncorrected = navigation_planning_backend::minimumSnapStopPiece(
      state, rounded_lower_bound);
  ASSERT_GT(uncorrected.getMaxJerRate(), jerk_limit);
  EXPECT_TRUE(navigation_planning::withinNumericalDynamicLimit(
      uncorrected.getMaxJerRate(), jerk_limit));
...
TEST(PlannerBackupBraking, InteriorSteadyDurationNeedsNoBoundaryCorrection) {
  navigation_math::StatePVAJ state = navigation_math::StatePVAJ::Zero();
  state.col(1).x() = 0.1;
  const double lower_bound =
      navigation_planning_backend::minimumSnapSteadyCruiseDuration(
          0.1, 2.0, 10.0, 0.05);
  const auto seed = navigation_planning_backend::makeBackupBrakingSeed(
      0.0, state, 1.0, 2.0, 10.0, 0.05, 0.0);
  ASSERT_TRUE(seed.feasible);
  EXPECT_DOUBLE_EQ(seed.duration_s, lower_bound);
}

TEST(PlannerBackupBraking, AbortDuringSteadyBoundaryCorrectionIsObserved) {
  navigation_math::StatePVAJ state = navigation_math::StatePVAJ::Zero();
  state.col(1).x() = 0.08839407447163837;
  int abort_checks = 0;
  const auto seed = navigation_planning_backend::makeBackupBrakingSeedWithAbort(
      0.0, state, 0.08839407447163837, 0.00012525139441244633, 1.0e6,
      0.001, 0.0, [&] { return ++abort_checks == 3; });
  EXPECT_FALSE(seed.feasible);
  EXPECT_EQ(seed.failure,
            navigation_planning_backend::StopFailureReason::kBudgetExhausted);
}

TEST(PlannerBackupBraking, MaterialMeasuredAccelerationIsNotLaundered) {
  navigation_math::StatePVAJ state = navigation_math::StatePVAJ::Zero();
  state.col(1).x() = 0.08839407447163837;
  state.col(2).x() = 0.00012525139441244633 + 1.0e-5;
  const auto seed = navigation_planning_backend::makeBackupBrakingSeed(
      0.0, state, 0.08839407447163837, 0.00012525139441244633, 1.0e6,
      0.001, 0.0);
  EXPECT_FALSE(seed.feasible);
  ...
  EXPECT_FALSE(navigation_planning::withinNumericalDynamicLimit(
      state.col(2).norm(), 0.00012525139441244633));
}
...
TEST(PlannerSpeedGovernor, AbortBeforeCurrentStopDoesNotStartPolynomialWork) {
  navigation_math::StatePVAJ state = navigation_math::StatePVAJ::Zero();
  navigation_planning::DynamicLimits dynamics;
  dynamics.vehicle.maximum_velocity_mps = 8.0;
  dynamics.vehicle.maximum_acceleration_mps2 = 4.0;
  ...
  EXPECT_EQ(result.failure,
            navigation_planning_backend::EvidenceSpeedFailure::kBudgetExhausted);
  EXPECT_EQ(abort_checks, 1);
}

TEST(PlannerBackupBraking, AbortIsObservedBetweenBoundedDurationTrials) {
  navigation_math::StatePVAJ state = navigation_math::StatePVAJ::Zero();
  state.col(1).x() = 2.0;
  state.col(2).x() = 1.5;
  int abort_checks = 0;
  const auto seed = navigation_planning_backend::makeBackupBrakingSeedWithAbort(
      0.0, state, 5.0, 2.0, 8.0, 0.05, 0.0,
      [&] { return ++abort_checks == 3; });
  EXPECT_FALSE(seed.feasible);
  EXPECT_EQ(seed.failure,
            navigation_planning_backend::StopFailureReason::kBudgetExhausted);
  EXPECT_EQ(abort_checks, 3);
}

TEST(PlannerBackupBraking, AbortIsObservedInsideTerminalAltitudeAdjustment) {
  navigation_math::StatePVAJ state = navigation_math::StatePVAJ::Zero();
  state.col(0).z() = 3.0;
  state.col(1).x() = 2.0;
  int abort_checks = 0;
  const auto seed =
      navigation_planning_backend::makeBackupBrakingSeedWithTerminalAltitudeAndAbort(
          0.0, state, 5.0, 2.0, 8.0, 0.05, 0.0, 3.0,
          [&] { return ++abort_checks == 4; });
  EXPECT_FALSE(seed.feasible);
  EXPECT_EQ(seed.failure,
            navigation_planning_backend::StopFailureReason::kBudgetExhausted);
  EXPECT_FALSE(seed.terminal_altitude_preserved);
  EXPECT_EQ(abort_checks, 4);
}

TEST(PlannerSpeedGovernor, AbortImmediatelyBeforeSuccessfulReturnIsContained) {
  navigation_math::StatePVAJ state = navigation_math::StatePVAJ::Zero();
  navigation_planning::DynamicLimits dynamics;
  dynamics.vehicle.maximum_velocity_mps = 8.0;
  dynamics.vehicle.maximum_acceleration_mps2 = 4.0;
  ...
  const auto result = navigation_planning_backend::evidenceAwareSpeedLimit(
      state, dynamics, {100.0, 100.0, 100.0, 100.0}, 0.05,
      [&] { return ++abort_checks == 9; });
  EXPECT_FALSE(result.sufficient);
  EXPECT_EQ(result.failure,
            navigation_planning_backend::EvidenceSpeedFailure::kBudgetExhausted);
  EXPECT_EQ(abort_checks, 9);
}
