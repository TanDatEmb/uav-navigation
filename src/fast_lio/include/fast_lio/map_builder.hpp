 ...
 *
 * Reference: frame_contract_v1.md, ieskf_math_review.md
 */
class MapBuilder {
 public:
  /**
   ...
   */
  BuilderStatus status() const { return status_; }

  /**
   * @brief Check if initialized and mapping.
   */
  bool isInitialized() const { return status_ == BuilderStatus::MAPPING; }

  /**
   * @brief Get IESKF instance.
   */
  std::shared_ptr<IESKF> getKF() const { return kf_; }

  /**
   * @brief Get LiDAR processor.
   */
  std::shared_ptr<LidarProcessor> getLidarProcessor() const { return lidar_processor_; }
