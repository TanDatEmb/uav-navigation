...
TEST(TrackingEnvelope, PhaseCDiagnosticRejectsMeasuredVehicleAheadOfReference) {
  ...
  // genuine reverse-envelope rejection; it must not be relabelled as forward
  // lag or made valid by the speed-dependent forward allowance.
  const auto result = evaluateTrackingEnvelope(
  ...
  EXPECT_NEAR(result.longitudinal_limit_m, 1.3503555875540325, 1.0e-12);
