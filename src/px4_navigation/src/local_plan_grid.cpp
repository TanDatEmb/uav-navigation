...
void LocalPlanGrid::MarkOccupied(const Eigen::Vector3d &position) {
    const Eigen::Vector3i index = px4_nav_common::math::WorldToIndex(position, origin_, resolution_);
    if (!px4_nav_common::math::IsIndexInBounds(index, size_)) {
        return;
    }

    const int address = px4_nav_common::math::IndexToAddress(index, size_);
    grid_[address] = 1;
}
...
void LocalPlanGrid::InflateObstacles(int inflation_radius_voxels) {
    ...
    for (int address = 0; address < total_voxels; ++address) {
        ...
        const Eigen::Vector3i current_index = px4_nav_common::math::AddressToIndex(address, size_);

        for (const auto &offset : stencil) {
            const Eigen::Vector3i inflated_index = current_index + offset;
            if (!px4_nav_common::math::IsIndexInBounds(inflated_index, size_)) {
                continue;
            }

            const int inflated_address = px4_nav_common::math::IndexToAddress(inflated_index, size_);
