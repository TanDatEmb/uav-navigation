...
Default is off. The runner writes injection parameters only for an explicit test invocation; production YAML has no enabling parameter. The hook never supplies a candidate, certificate, freshness witness, command authority, or retained-validation result. Its one-shot `consumed` bit is test-only bookkeeping. Normal disabled execution skips the eligibility work. The status substitution adds no persistent product authority field.
