...
The analyzer reports `INJECTION_ALREADY_CONSUMED` as its derived terminal one-shot state when exactly one applied event exists. This is not a second runtime event. A repeated application would fail its `applied_exactly_once` assertion.
