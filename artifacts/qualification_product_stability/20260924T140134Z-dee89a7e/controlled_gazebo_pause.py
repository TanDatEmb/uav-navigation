#!/usr/bin/env python3
"""Run one diagnostic SITL and pause only its exact Gazebo server PID once."""
import json
import os
...
OUT = ROOT / '.artifacts/runtime/qpst-controlled-pause-index.json'
LOG = Path('/tmp/qpst-controlled-pause-run.log')


def gazebo_pid(session: Path) -> int | None:
    registry = session / 'processes.json'
    if not registry.exists():
        return None
    try:
        groups = {int(r['pgid']) for r in json.loads(registry.read_text())['processes']
                  if r.get('role') == 'px4_gazebo'}
    ...
    for proc in Path('/proc').glob('[0-9]*'):
        try:
            ...
            if pgrp in groups and cmd.startswith('gz sim -r -s '):
...
def main() -> None:
    ...
    command = ['python3', 'tools/runtime/run_state_transport_cohort.py',
               '--count', '1', '--label', 'qpst-gazebo-pause',
