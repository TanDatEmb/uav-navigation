...
The scanner found **368 declarations**, triaged as **239 behavioral candidates** and **79 diagnostic/test candidates**; the rest are infrastructure/config. Of the 239, **22 KEEP, 3 MOVE, 214 UNRESOLVED**. `DERIVE/MERGE/DELETE` are zero because exact information conservation is not yet proved. The `hold_handover_attempts_` counter moved to diagnostic after its selected writer/read sites showed logging only (`navigation_mode_node.cpp:2871,2900,2924-2936`). These are **scoped candidate counts**, not a whole-product behavioral total. Aggregates such as `RouteProgress.state_`, `CandidateBundle`, execution-state lease payload, mapping model state, mission data, and nested adapter caches must be expanded before claiming 100%.

Every row includes declaration path/line and Git blob hash. Selected-path writer/reader/lifetime evidence is filled for 68 rows, including all 17 scanned `MissionController` fields and selected adapter recovery/completion fields; other rows remain `UNRESOLVED` until a complete alias/call graph and transition proof exists. Even filled rows do not assert exhaustive writer coverage or target equivalence. A named field is not a duplicate solely because its name resembles another. `tools/extract_state.py` verifies each input blob against `TARGET_SHA` on regeneration, so audit-only commits can advance HEAD without changing the analyzed product source.
