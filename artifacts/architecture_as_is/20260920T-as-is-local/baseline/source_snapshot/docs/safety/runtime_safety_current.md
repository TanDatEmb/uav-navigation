...
|---|---|---|
| TB-003 | Historical reduction `2 -> 1`; worktree restores reference `2` as an unpromoted trial. Owner: mapping/planning; second-pass compute may reduce remaining solve time. Budget/inflation/collision gates unchanged. | Keep assurance debt open until matched dense snapshot/dataset/SITL `1` vs `2` establishes complete-feasible rate and tails; withdrawal by reviewed revert, no silent gate/budget change. See current trial in the canonical report and archived register. |
...
- Contract/behavior: the shared PVAJ scalar is a search estimate, not stop authorization. Require a concrete minimum-snap stop, bounded extrema and polynomial support; corridor/world remain independent. Measured PVAJ and physical limits govern; nominal cruise cannot erase derivatives.
- Evidence/removal: native `PlannerBackupBraking`/`PlannerSpeedGovernor` counterexamples and positive-state tests. Keep until a verified replacement preserves full-state stop, support and world authorization; integration/recorded-data evidence remains open. Lineage: DEC-20260903-001.
...
- Planning: CIRI reference2 (TB-003), loader/config agree;80ms/400ms/dynamics/certificates/leases unchanged; SAFE known-free/FAST explicit UNKNOWN, both forbid OCCUPIED/OOM.
...
- Verify unequal/reversed/multiple-junction oracle, cap/corner negatives, backend/full
  regression and Release. Lineage DEC-20260828-089; [current action/evidence](../reports/feasible_checkpoint_5mps_diagnostic_2026-09-17.md#nonuniform-corridor-junction-correction--2026-09-18).
