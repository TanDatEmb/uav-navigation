...
inline const Eigen::Matrix3d& c_flu_frd() {
  static const Eigen::Matrix3d matrix =
      Eigen::DiagonalMatrix<double, 3>(1.0, -1.0, -1.0);
  return matrix;
}

inline Eigen::Vector3d fluToFrd(const Eigen::Vector3d& value_flu) {
  return c_flu_frd() * value_flu;
}

inline Eigen::Vector3d frdToFlu(const Eigen::Vector3d& value_frd) {
  return c_flu_frd() * value_frd;
}
