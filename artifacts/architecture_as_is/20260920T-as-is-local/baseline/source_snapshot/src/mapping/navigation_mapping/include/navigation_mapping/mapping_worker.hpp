...
// Bounded single-owner execution primitive for mapping observations. The process callback is
...
class MappingWorker {
 public:
  ...
  bool submitFromWaiting(Observation observation) {
    ...
    if (!accepting_ || fatal_ || resetting_) {
  ...
  // Establish a reset barrier for the sole mutable-map owner. READY work is
  // discarded immediately; an IN_FLIGHT callback is allowed to finish before
  // the caller resets the owned map. While the barrier is active no new
  // observation can enter READY. The order key is reset together with the
  // lifecycle so a new localization epoch can restart its source timestamps.
  void reset() {
    std::unique_lock lock(mutex_);
