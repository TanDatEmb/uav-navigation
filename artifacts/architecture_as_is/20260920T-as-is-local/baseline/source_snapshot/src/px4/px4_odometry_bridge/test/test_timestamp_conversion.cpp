...
TEST(TimestampConversionTest, ValidMonotonicSimulationTimestamps) {
  ...
  const auto result = converter.convert(
      1'000'000'000, 1'020'000'000, TimestampMappingMode::kSimulationIdentity, 7);
  ...
  EXPECT_TRUE(converter.convert(
      1'001'000'000, 1'021'000'000,
      TimestampMappingMode::kSimulationIdentity, 7).valid);
}

TEST(TimestampConversionTest, ValidMonotonicRealtimeTransportTimestamps) {
  TimestampConverter converter(150'000'000);
  const auto result = converter.convert(
      1'000'000'000, 1'020'000'000,
      TimestampMappingMode::kRealtimeTransport, 3);
  EXPECT_TRUE(result.valid);
  EXPECT_EQ(result.source_domain, "ROS_SYSTEM_TIME");
  EXPECT_EQ(result.target_domain, "PX4_TIME_VIA_UXRCE_TIMESYNC");
  EXPECT_EQ(result.reason, "VALID_REALTIME_UXRCE_MAPPING");
}

TEST(TimestampConversionTest, ResolvesInputDomainOnlyForMatchingRosClock) {
  EXPECT_EQ(timestampMappingModeFor(true, "simulation_time"),
            TimestampMappingMode::kSimulationIdentity);
  EXPECT_EQ(timestampMappingModeFor(false, "ros_time"),
            TimestampMappingMode::kRealtimeTransport);
  EXPECT_EQ(timestampMappingModeFor(false, "system_time"),
            TimestampMappingMode::kRealtimeTransport);
  EXPECT_EQ(timestampMappingModeFor(true, "ros_time"),
            TimestampMappingMode::kUnresolved);
  EXPECT_EQ(timestampMappingModeFor(false, "sensor_time"),
            TimestampMappingMode::kUnresolved);
}
...
TEST(TimestampConversionTest, EqualSimulationPublicationEpochIsMonotonic) {
  ...
  ASSERT_TRUE(converter.convert(
      ...
      TimestampMappingMode::kSimulationIdentity, 7).valid);
  const auto result = converter.convert(
      1'001'000'000, 1'020'000'000,
      TimestampMappingMode::kSimulationIdentity, 7);
  EXPECT_TRUE(result.valid);
...
TEST(TimestampConversionTest, EqualMeasurementEpochIsSuppressedWithoutFailure) {
  ...
  ASSERT_TRUE(converter.convert(
      ...
      TimestampMappingMode::kSimulationIdentity, 1).valid);
  const auto result = converter.convert(
      2'000'000'999, 2'002'000'999,
      TimestampMappingMode::kSimulationIdentity, 1);
...
TEST(TimestampConversionTest, SampleRegressionIsRejectedAndCounted) {
  TimestampConverter converter(150'000'000);
  ASSERT_TRUE(converter.convert(
      2'001'000'000, 2'002'000'000,
      TimestampMappingMode::kSimulationIdentity, 1).valid);
  const auto result = converter.convert(
      2'000'000'000, 2'003'000'000,
      TimestampMappingMode::kSimulationIdentity, 1);
  EXPECT_FALSE(result.valid);
...
TEST(TimestampConversionTest, PublicationRegressionIsRejectedAndCounted) {
  TimestampConverter converter(150'000'000);
  ASSERT_TRUE(converter.convert(
      2'000'000'000, 2'002'000'000,
      TimestampMappingMode::kSimulationIdentity, 1).valid);
  const auto result = converter.convert(
      2'001'000'000, 2'001'000'999,
      TimestampMappingMode::kSimulationIdentity, 1);
  EXPECT_FALSE(result.valid);
...
TEST(TimestampConversionTest, TimestampMappingGenerationChangeStartsANewEpoch) {
  TimestampConverter converter(150'000'000);
  ASSERT_TRUE(converter.convert(
      3'000'000'000, 3'001'000'000,
      TimestampMappingMode::kSimulationIdentity, 1).valid);
  EXPECT_TRUE(converter.convert(
      1'000'000'000, 1'001'000'000,
      TimestampMappingMode::kSimulationIdentity, 2).valid);
...
TEST(TimestampConversionTest, StaleFutureUnresolvedAndZeroAreFailClosed) {
  ...
  EXPECT_EQ(stale.convert(
      1'000'000'000, 1'200'000'001,
      TimestampMappingMode::kSimulationIdentity, 1).reason,
            "TIMESTAMP_STALE");
  TimestampConverter future(150'000'000);
  EXPECT_EQ(future.convert(
      1'200'000'001, 1'000'000'000,
      TimestampMappingMode::kSimulationIdentity, 1).reason,
            "TIMESTAMP_FUTURE");
  TimestampConverter unresolved(150'000'000);
  EXPECT_EQ(unresolved.convert(
      1'000'000'000, 1'001'000'000,
      TimestampMappingMode::kUnresolved, 1).reason,
            "TIME_DOMAIN_UNRESOLVED");
  TimestampConverter zero(150'000'000);
  EXPECT_EQ(zero.convert(
      0, 1'000'000,
      TimestampMappingMode::kSimulationIdentity, 1).reason,
