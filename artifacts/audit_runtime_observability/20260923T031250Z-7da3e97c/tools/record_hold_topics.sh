#!/usr/bin/env bash
# Audit-only, read-only ROS topic recorder. Run beside a focused SITL scenario.
set -euo pipefail
if [[ $# -ne 1 ]]; then
...
required=(/fmu/in/vehicle_command /fmu/in/vehicle_command_mode_executor /fmu/out/vehicle_command_ack /fmu/out/mode_completed /fmu/out/vehicle_status_v1)
ready=0
