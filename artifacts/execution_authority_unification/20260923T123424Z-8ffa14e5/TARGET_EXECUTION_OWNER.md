...
`ExecutionAuthority` is in `navigation_execution`, with one internal mutex and private `{goal, bundle}` active and `{goal, bundle, activation_ns}` staged records. Its immutable `ExecutionAuthoritySnapshot` carries both records, admission epochs, world identity, lineage/version and typed lifecycle. `ExecutionEpisodeSnapshot` is a read-only compatibility/telemetry projection, never a second owner. The full goal is supplied before commit/stage and paired with the bundle in the transaction. RuntimeNode keeps only its desired `active_goal_`; the execution goal and epoch are read from the owner.

`setAdmissionGoalEpoch()` names the stale-result fence honestly. It can advance to N+1 while the predecessor N remains active. Candidate commit and staged activation set the active record and lifecycle inside the one owner mutex. The finalizer still rolls back active/staged/lifecycle if planner-history acknowledgement fails. A lower generation cannot replace the current active record with a newer queue transaction ID; same-generation identity-preserving replacement remains supported.

Publication captures an owner snapshot, evaluates the polynomial outside the owner mutex, then calls `publishIfCurrent()` with the exact bundle. The owner rechecks pointer, world identity and available exposure under its mutex; the bounded callback receives the authorized lifecycle for telemetry. The existing ROS publish callback remains inside this final short critical section to preserve publication-versus-revocation linearization. No planner solve, world sweep or candidate construction was moved under that mutex.

One `publish_lock_wait_us_` atomic is diagnostic-only. `publishIfCurrent()` writes it; the diagnostics publisher reads it; no admission, publication or safety branch consumes it. It preserves a lock-wait observation needed for this cut's timing comparison, and is not an execution authority copy.
