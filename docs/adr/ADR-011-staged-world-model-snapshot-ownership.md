...
runtime instead owns one mutable `rog_map::ROGMapROS` inside
`super_navigation_node`; the same mutually-exclusive callback first updates the
map and then runs SUPER.
...
ROG-Map is mutable and has no read/write synchronization contract. SUPER has
49 direct calls through 19 ROG methods across A*, corridor generation and
trajectory safety. Running a map-update callback concurrently with those
queries would therefore be a data race. Holding a shared/exclusive map lock for
an entire solve would avoid the race but preserve mapping starvation and stale
map revisions. Neither is the target architecture.
...
Introduce a product-owned `WorldModelView` contract and a ROG-backed adapter.
...
occupied-point extraction required by CIRI. SUPER receives this contract, not
`ROGMapROS`.
