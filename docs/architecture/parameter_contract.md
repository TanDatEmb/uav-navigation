...
| `traj_opt/boundary/max_*` / `dynamic_limit_tolerance_ratio` | Safety envelope có allowance bounded | `max_*` giữ với đơn vị vật lý rõ: m/s, m/s², m/s³, rad/s và N. Mission chỉ được hạ, không được nâng; `dynamic_limit_tolerance_ratio` là allowance V/A/J chung, hiện giới hạn 0.75 và bị reject nếu âm/không hữu hạn/vượt trần. Không áp dụng allowance này cho flatness, PX4 hay hardware gate. |
