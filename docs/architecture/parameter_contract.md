...
|---|---|---|
| `navigation_runtime` topics, frames, `data_freshness_window_s` | Đang dùng | Giữ; frame và thời gian phải kiểm tra ở boundary. Đây là cửa sổ freshness chung của runtime và command lease, không phải bản sao của các ngưỡng PX4/bridge. Nếu không có mission contract, planner mặc định `RequireKnownFree`; chỉ mission hợp lệ mới có thể cấp policy MAIN `allow_unknown`. |
| `planner_rate_hz`, `command_rate_hz`, `mapping_snapshot_publication_period_s`, `command_stream_timeout_s`, `stopped_recovery_timeout_s` | Đã chuyển thành product-locked constants | Không còn là ROS parameters hay YAML producer. `navigation_planning::PlanningTimingContract` sở hữu các cadence và safety windows (`10 Hz`, `50 Hz`, `0.10 s`, `0.10 s`, `5.0 s`); runtime chỉ dùng các giá trị typed này. Việc giữ lại các núm exact-equality chỉ tạo surface giả configurable và làm người vận hành tưởng có thể đổi semantics. |
| `input_pair_max_skew_s` | Đã xóa | Pairing legacy thực tế yêu cầu timestamp tuyệt đối, không dùng skew cấu hình. |
| `deployment_profile` | Necessary deployment safety restriction | Giữ trên ROS/YAML để buộc provenance môi trường. `sitl` là profile duy nhất hiện được phép khởi động; `hardware` bị chặn cho tới khi có artifact visibility bất biến và verifier runtime. Không thay bằng boolean hoặc xóa vì sẽ mất fail-closed hardware restriction. |
...
| `traj_opt/boundary/max_*` | Safety envelope strict | `max_*` giữ với đơn vị vật lý rõ: m/s, m/s², m/s³, rad/s và N. Mission chỉ được hạ, không được nâng. Key tỷ lệ tolerance cũ đã bị xóa và sự hiện diện của nó làm startup thất bại; hard V/A/J chỉ có fixed floating-point ULP accounting. Không áp dụng tolerance ngầm cho flatness, PX4 hay hardware gate. |
...
| Mapping ESDF/frontier/ROS callback/visualization/raycasting/virtual plane | Capability config | Product profile chỉ giữ raycasting, sliding-map và virtual-plane contract đang có consumer; ESDF/frontier/visualization/load-PCD/unknown-inflation disabled surface đã bỏ khỏi YAML sản phẩm. API tương thích của vendor vẫn nằm trong package adapter và không được coi là product behavior. |
...
| estimator `propagated_odometry.publish_rate_hz` / runtime command scheduler | Producer rate và consumer tick | Giữ độc lập; producer rate là contract dữ liệu, còn runtime command tick là product-locked `PlanningTimingContract::kCommandRateHz`, không phải ROS parameter. Không dùng một rate để suy ra rate kia. |
