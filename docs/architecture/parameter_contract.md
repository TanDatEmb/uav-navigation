...
| `planner_rate_hz`, `command_rate_hz`, `mapping_snapshot_publication_period_s`, `command_stream_timeout_s`, `stopped_recovery_timeout_s` | Đã chuyển thành product-locked constants | Không còn là ROS parameters hay YAML producer. `navigation_planning::PlanningTimingContract` sở hữu các cadence và safety windows (`5 Hz`, `50 Hz`, `0.10 s`, `0.10 s`, `5.0 s`); runtime chỉ dùng các giá trị typed này. Việc giữ lại các núm exact-equality chỉ tạo surface giả configurable và làm người vận hành tưởng có thể đổi semantics. |
