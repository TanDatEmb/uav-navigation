...
| `traj_opt/boundary/max_*` / `dynamic_limit_tolerance_ratio` | Safety envelope strict; compatibility key | `max_*` giữ với đơn vị vật lý rõ: m/s, m/s², m/s³, rad/s và N. Mission chỉ được hạ, không được nâng. `dynamic_limit_tolerance_ratio` chỉ còn để phát hiện cấu hình cũ và phải bằng đúng `0.0`; mọi giá trị khác 0 bị reject, không có allowance V/A/J. Không áp dụng một tolerance ngầm cho flatness, PX4 hay hardware gate. |
