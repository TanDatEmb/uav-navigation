...
### 2026-08-28 - Reject regressions in absolute certified MAIN horizon

- **Owner:** Receding-horizon command admission and runtime continuity.
- **Scope:** For the same localization, goal and request identity, a newly
  solved command may not replace the current command when its absolute
  switch-to-BACKUP time is earlier. The current command is retained only while
  its MAIN interval is still live and after its exact remaining role schedule
  passes yaw, route-regression and swept-volume validation against the latest
  published immutable world.
- **Safety impact:** No occupancy, UNKNOWN, OUT_OF_MAP, dynamics, yaw or route
  gate is relaxed. A current bundle that fails any latest-world certificate is
  never retained by this rule; normal candidate authorization or fail-closed
  runtime behavior remains authoritative. A world publication after this
  read-only comparison is handled by the existing runtime recertification
  boundary, so retention does not mint a new or stale certificate.
- **Evidence:** Rolling three-column artifacts repeatedly show accepted
  bundles switching MAIN to BACKUP after roughly 1--1.5 s, followed by several
  seconds of braking and another acceleration. Existing admission checked
  geometric safety and route regression but did not compare absolute MAIN end
  times, allowing a safe new solve to shorten the executable nominal horizon.
- **Removal/review condition:** Keep until command admission is replaced by a
  richer lexicographic continuity/progress contract with equivalent
  latest-world recertification. Remove if repeated representative SITL shows
  retained stale geometry delays necessary obstacle deformation, increases
  route error, or worsens stopping continuity.
- **Verification:** Run backend trajectory tests, runtime planner/FSM tests and
  a Release build, then compare repeated unchanged three-column SITL runs for
  retained-horizon events, committed generations, MAIN/BACKUP transitions,
  central-route low-speed episodes, route error, clearance and planner tails.
