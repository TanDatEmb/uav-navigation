...
### 2026-08-28 - Calibrate three-column mission dynamics to 5/3/8

- **Owner:** Three-column speed and multi-waypoint mission envelopes.
- **Scope:** Keep mission velocity at 5 m/s and raise only the mission-level
  acceleration and jerk ceilings from 2/4 to 3/8 m/s^2 and m/s^3. Planner
  product limits remain 12/12/30; all continuous dynamics, flatness, world,
  route, yaw, splice and PX4 tracking certificates remain unchanged.
- **Safety impact:** This is a behavior calibration, not a gate bypass. The
  candidate must still satisfy the new finite mission envelope and PX4's
  unchanged 0.75 m tracking envelope. The values match the already declared
  `single_pillar_speed_pv` mission and stay substantially below the product
  physical envelope. Any tracking, clearance, altitude or continuity
  regression requires rollback rather than another threshold increase.
- **Evidence:** In exact-profile artifacts
  `.artifacts/runtime/external-mode-check-20260828T142323-1223796` and
  `.artifacts/runtime/external-mode-check-20260828T143719-1237126`, respectively
  47 of 91 and 45 of 81 EXP failures had finite optimized extrema within
  5/3/8. Candidate jerk p95 was 8.553 and 6.293 m/s^3; acceleration p95 was
  2.065 and 2.058 m/s^2. The old 2/4 limits therefore rejected a recurrent,
  measured class of near-feasible replans rather than rare outliers.
- **Removal/review condition:** Retain only if repeated two-waypoint and
  multi-waypoint three-column runs improve accepted replans and cruise
  continuity without degrading tracking, altitude, yaw, route error,
  clearance or safety transitions. Recorded-data processing evidence remains
  separately required and cannot certify flight behavior.
- **Verification:** Run mission/runtime contract tests and an authoritative
  Release build, then repeated unchanged `long_three_pillars_speed` and
  `long_three_pillars_multiwaypoint` SITL. Compare EXP outcomes, commits,
  MAIN/BACKUP transitions, low-speed episodes, actual/setpoint V/A/J,
  tracking error, altitude, yaw, route error, clearance and completion.
