...
| Xacro expansion + `check_urdf` | 0 | pass; explicit SIM mount expands to the canonical three-link tree |
| Missing-mount launch validation | 1 | pass; standalone launch fails with the required `livox_mount_xyz` argument error |
...
## Static description validation

The P0.1 Xacro was expanded with:

```text
livox_mount_xyz="0 0 0.28"
livox_mount_rpy="0 0 0"
```

The system package could not be installed because this terminal has no sudo
credential; the same ROS Jazzy `xacro` 2.1.1 package was unpacked in `/tmp`
for this validation, while `check_urdf` was run from the installed ROS tree.

`check_urdf` passed and reported exactly one rooted chain:
...
The resolved joint origins were:

```text
base_link -> livox_frame:             0 0 0.28
livox_frame -> livox_imu_frame:       0.011 0.02329 -0.04412
```

The missing-mount standalone launch was also exercised and failed clearly
before starting the node: `Included launch description missing required
argument 'livox_mount_xyz'`. The direct Xacro default is an intentionally
invalid sentinel, not a zero mount; therefore direct expansion without a
mount also fails `check_urdf`.
...
- [x] Xacro expands and `check_urdf` validates the canonical static tree
- [x] Missing-mount standalone launch fails with a required-argument error
